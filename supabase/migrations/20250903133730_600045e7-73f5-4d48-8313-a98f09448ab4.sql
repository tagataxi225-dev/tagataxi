-- Phase 2: Complete RLS Policies for all remaining tables
...
FOR ALL USING (auth.uid() = referred_by OR auth.uid() = referred_user_id);
...
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'subscription_plans') THEN
    EXECUTE 'CREATE POLICY "subscription_plans_public_read" ON public.subscription_plans
    FOR SELECT USING (is_active = true)';
    
    EXECUTE 'CREATE POLICY "subscription_plans_admin_manage" ON public.subscription_plans
    FOR ALL USING (is_current_user_admin())';
  END IF;
END $$;