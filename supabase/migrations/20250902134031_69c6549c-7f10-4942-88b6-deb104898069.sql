-- Phase 1: Critical Payment Data Protection - FIXED
-- Create audit logging table for payment access
...
-- Create audit function for escrow_payments (without problematic trigger)
...
-- Fix rental_subscription_payments table if it exists
DO $$
BEGIN
  -- Check if rental_subscription_payments table exists
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'rental_subscription_payments') THEN
    -- Create audit logging for rental payments
    ...
    DROP POLICY IF EXISTS "System can manage rental payments" ON public.rental_subscription_payments;
    
    -- Create partner-only access policy
    CREATE POLICY "Partners can view their own rental payments"
    ...
    -- Service role can manage for system operations
    CREATE POLICY "Service role can manage rental payments"
    ON public.rental_subscription_payments
    FOR ALL
    USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');
