-- COMPREHENSIVE SECURITY HARDENING - SAFE VERSION
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'sensitive_data_access_logs'
  ) THEN
...
-- 2. Create secure audit logging function
...
-- 3. Create secure chauffeur data access function
...
-- 4. Create secure driver location access function
...
-- 5. Fix database function search paths (Critical Security Issue)
...
-- 6. Create financial operations audit trigger
...
-- Apply financial audit triggers safely
DO $$
BEGIN
...
-- 7. Clean up duplicate partenaires policies safely
DO $$
BEGIN
  -- Remove duplicate French policies if they exist
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'partenaires' AND policyname = 'Admins peuvent voir tous les partenaires') THEN
    DROP POLICY "Admins peuvent voir tous les partenaires" ON public.partenaires;
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'partenaires' AND policyname = 'Partenaires peuvent gérer leur propre profil') THEN
    DROP POLICY "Partenaires peuvent gérer leur propre profil" ON public.partenaires;
  END IF;
END $$;