-- ==============================================
-- MIGRATION DES DONNÉES CORRIGÉE - PHASE 2 (FINALE)
-- ==============================================

-- 1. CRÉER LA TABLE driver_service_associations (si elle n'existe pas déjà)
...
-- Contrainte unique séparée
CREATE UNIQUE INDEX IF NOT EXISTS idx_driver_service_unique_active 
...
DROP POLICY IF EXISTS "driver_service_associations_driver_access" ON public.driver_service_associations;
DROP POLICY IF EXISTS "driver_service_associations_admin_access" ON public.driver_service_associations;
...
-- 2. CRÉER LA TABLE DE LOGS DE MIGRATION (si elle n'existe pas déjà)
...
DROP POLICY IF EXISTS "migration_logs_admin_access" ON public.data_migration_logs;
CREATE POLICY "migration_logs_admin_access" 
    ON public.data_migration_logs FOR ALL 
    USING (is_current_user_admin())
    WITH CHECK (is_current_user_admin());

-- 3. FONCTION DE MAPPING BASÉE SUR LES COLONNES EXISTANTES
...
-- 4. AJOUTER COLONNES DE COMPATIBILITÉ DANS CHAUFFEURS
...
-- 5. MIGRER LES CHAUFFEURS EXISTANTS (SANS RÉFÉRENCE À vehicle_class)
DO $$
DECLARE
...
BEGIN
    -- Parcourir tous les chauffeurs actifs non encore migrés
    ...
    LOOP
        ...
        IF service_id IS NULL THEN
            SELECT id INTO service_id
            FROM public.service_configurations
            WHERE service_name = 'taxi_standard' AND is_active = true
            LIMIT 1;
...
-- 6. METTRE À JOUR LES CHAUFFEURS MIGRÉS
...
-- 7. FONCTION POUR VÉRIFIER LE STATUT DE MIGRATION
...
-- 8. CRÉER UNE VUE POUR LA COMPATIBILITÉ
...
SELECT 
    ...
    -- Service effectif
    COALESCE(sc.service_name, c.migrated_service_type, 'taxi_standard') as effective_service
...
INSERT INTO public.security_audit_logs (
    ...
    true,
    jsonb_build_object(
        'migration_phase', 'data_migration_final_completed',
        'timestamp', now()
    )
);

-- 10. AFFICHER LE STATUT DE MIGRATION
