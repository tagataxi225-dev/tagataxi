-- Migration pour le système de demandes de comptes équipe
...
-- 4. Politiques RLS pour team_requests
...
FOR SELECT
TO authenticated
USING (has_permission(auth.uid(), 'teams_admin'));

-- Les administrateurs peuvent mettre à jour les demandes (approuver/rejeter)
...
-- Les administrateurs peuvent voir tous les comptes équipe
CREATE POLICY "Admins can view all team accounts"
ON public.team_accounts
FOR SELECT
TO authenticated
USING (has_permission(auth.uid(), 'teams_admin'));
...
-- Les administrateurs peuvent mettre à jour tous les comptes équipe
CREATE POLICY "Admins can update all team accounts"
ON public.team_accounts
FOR UPDATE
TO authenticated
USING (has_permission(auth.uid(), 'teams_admin'));
