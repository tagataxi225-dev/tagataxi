-- ==============================================
-- MIGRATION DES DONNÉES CORRIGÉE - PHASE 2
-- ==============================================

-- 1. CRÉER LA TABLE driver_service_associations AVEC SYNTAXE CORRIGÉE
...
-- Contrainte unique séparée pour un chauffeur ne peut avoir qu'un seul service actif
...
-- 3. FONCTION DE MAPPING SIMPLIFIÉE
...
AS $$
BEGIN
    -- Mapper selon vehicle_type en priorité
    IF p_vehicle_type IS NOT NULL THEN
        CASE LOWER(p_vehicle_type)
            ...
            WHEN 'taxi', 'car' THEN 
                CASE LOWER(COALESCE(p_vehicle_class, 'standard'))
...
-- 5. MIGRER LES CHAUFFEURS EXISTANTS
DO $$
DECLARE
...
BEGIN
    ...
    LOOP
        ...
        IF NOT EXISTS (
            SELECT 1 FROM public.driver_service_associations 
            WHERE driver_id = driver_record.user_id AND is_active = true
        ) THEN
...
INSERT INTO public.security_audit_logs (
    ...
    true,
    jsonb_build_object(
        'migration_phase', 'data_migration_completed',
        'timestamp', now()
    )
);

-- 9. CRÉER UNE VUE POUR LA COMPATIBILITÉ
...
-- Commentaires
COMMENT ON TABLE public.driver_service_associations IS 
'Table des associations entre chauffeurs et services dans le nouveau système';
