...
-- Note: available_drivers_summary is a view, not a table, so we can't add RLS policies
-- We'll need to secure the underlying tables instead
...
DROP POLICY IF EXISTS "escrow_participants_access" ON public.escrow_transactions;

CREATE POLICY "escrow_participants_only" 
...
DROP POLICY IF EXISTS "escrow_payments_participants" ON public.escrow_payments;

CREATE POLICY "escrow_payments_participants_only" 
