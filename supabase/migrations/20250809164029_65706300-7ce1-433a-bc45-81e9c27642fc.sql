-- Création du système de notifications admin vers utilisateurs
...
CREATE POLICY "Admins can manage notification types" ON public.admin_notification_types
  FOR ALL USING (has_permission(auth.uid(), 'user_notifications_admin'));
...
CREATE POLICY "Notification managers can view templates" ON public.admin_notification_templates
  FOR SELECT USING (has_permission(auth.uid(), 'user_notifications_write') OR has_permission(auth.uid(), 'user_notifications_admin'));
...
CREATE POLICY "Notification managers can manage their notifications" ON public.admin_notifications
  FOR ALL USING (has_permission(auth.uid(), 'user_notifications_write') AND sender_id = auth.uid());
...
CREATE POLICY "Admins can view all user notifications" ON public.user_notifications
  FOR SELECT USING (has_permission(auth.uid(), 'user_notifications_admin'));
