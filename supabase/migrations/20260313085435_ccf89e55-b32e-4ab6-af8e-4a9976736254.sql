-- Fix find_nearby_drivers: LEFT JOIN partner_drivers + grace mode for drivers without wallet/subscription
...
BEGIN
  ...
  -- ✅ FIX: LEFT JOIN instead of INNER JOIN — don't exclude drivers without partner entry
  LEFT JOIN partner_drivers pd ON pd.driver_id = dl.driver_id AND pd.status = 'active'
  ...
  WHERE 
    ...
    -- ✅ FIX: Grace mode — allow drivers through if they have no subscription AND no wallet entry yet
    AND (
      COALESCE(ds.rides_remaining, 0) > 0
      OR COALESCE(w.balance, 0) >= 500
      OR (ds.id IS NULL AND w.id IS NULL)
    )
