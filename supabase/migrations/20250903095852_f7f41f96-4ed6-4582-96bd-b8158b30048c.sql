-- Créer la fonction RPC manquante pour la recherche intelligente de lieux
-- Cette fonction remplace la recherche de base par une recherche avancée avec phonétique
...
CREATE INDEX IF NOT EXISTS idx_intelligent_places_location ON public.intelligent_places USING GIST(ll_to_earth(latitude, longitude));
  ...
  IF search_query = '' THEN
    ...
    SELECT 
      ...
      CASE 
        ...
        THEN round(earth_distance(ll_to_earth(user_latitude, user_longitude), ll_to_earth(p.latitude, p.longitude)))::INTEGER
        ELSE NULL
    ...
    (
      ...
      CASE 
        WHEN user_latitude IS NOT NULL AND p.latitude IS NOT NULL THEN
          GREATEST(0, (1 - (earth_distance(ll_to_earth(user_latitude, user_longitude), ll_to_earth(p.latitude, p.longitude)) / 10000))) * 0.1
        ELSE 0.1
      END
...
-- Insérer des données de test massives pour Kinshasa (2000+ lieux réels)
...
-- Ambassades (zone diplomatique)
('Ambassade des États-Unis', 'government', 'ambassade', 'Kinshasa', 'Gombe', 'Centre-ville', 'Avenue des Aviateurs', -4.3060, 15.3100, 5, 75, true, '{"US Embassy"}'),
('Ambassade de France', 'government', 'ambassade', 'Kinshasa', 'Gombe', 'Centre-ville', 'Avenue de l''Indépendance', -4.3070, 15.3110, 5, 75, true, '{}'),
...
-- Restaurants et divertissement
('Restaurant Chez Maman Colonel', 'restaurant', 'restaurant', 'Kinshasa', 'Gombe', 'Centre-ville', NULL, -4.3065, 15.3115, 5, 70, true, '{}'),
('Cercle Élysée', 'entertainment', 'club', 'Kinshasa', 'Gombe', 'Centre-ville', NULL, -4.3075, 15.3125, 5, 65, true, '{}'),

-- Quartiers étendus (Kalamu)
('UPN', 'district', 'quartier', 'Kinshasa', 'Kalamu', 'UPN', NULL, -4.3750, 15.2950, 3, 75, true, '{}'),
('Salongo', 'district', 'quartier', 'Kinshasa', 'Kalamu', 'Salongo', NULL, -4.3680, 15.2880, 3, 70, true, '{}'),
('Kasa-Vubu', 'district', 'quartier', 'Kinshasa', 'Kalamu', 'Kasa-Vubu', NULL, -4.3720, 15.2920, 3, 78, true, '{}'),

-- Plus de rues importantes  
('Avenue de la Liberation', 'transport', 'avenue', 'Kinshasa', 'Kalamu', 'Victoire', 'Avenue de la Liberation', -4.3700, 15.2900, 4, 70, true, '{"Liberation"}'),
('Avenue Kasa-Vubu', 'transport', 'avenue', 'Kinshasa', 'Kalamu', 'Kasa-Vubu', 'Avenue Kasa-Vubu', -4.3720, 15.2920, 4, 72, true, '{}'),
('Avenue des Cliniques', 'transport', 'avenue', 'Kinshasa', 'Gombe', 'Centre-ville', 'Avenue des Cliniques', -4.3090, 15.3190, 4, 68, true, '{}');
