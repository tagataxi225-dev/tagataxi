...
CREATE TABLE IF NOT EXISTS rental_equipment_types (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  name text NOT NULL,
  category text NOT NULL, -- 'comfort', 'safety', 'technology', 'luxury'
  icon text,
  description text,
...
CREATE POLICY IF NOT EXISTS "Anyone can view city pricing" 
ON rental_city_pricing FOR SELECT 
USING (true);

CREATE POLICY IF NOT EXISTS "Anyone can view equipment types" 
...
-- 7. Ajout de trigger pour mise à jour automatique
...
DROP TRIGGER IF EXISTS update_rental_city_pricing_updated_at ON rental_city_pricing;
CREATE TRIGGER update_rental_city_pricing_updated_at
  BEFORE UPDATE ON rental_city_pricing
  FOR EACH ROW
  EXECUTE FUNCTION update_rental_updated_at();
