...
-- Enhanced Support Tickets (extend existing)
CREATE TABLE public.enhanced_support_tickets (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Zone Analytics (if service_zones exists, reference it)
CREATE TABLE public.zone_analytics (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  zone_name TEXT NOT NULL, -- Use name instead of FK since service_zones may have different structure
...
ALTER TABLE public.enhanced_support_tickets ENABLE ROW LEVEL SECURITY;
...
-- RLS Policies for enhanced_support_tickets
CREATE POLICY "Users can view their own tickets" ON public.enhanced_support_tickets
  FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can create tickets" ON public.enhanced_support_tickets
...
CREATE INDEX idx_enhanced_support_tickets_user_id ON public.enhanced_support_tickets(user_id);
CREATE INDEX idx_enhanced_support_tickets_status ON public.enhanced_support_tickets(status);
CREATE INDEX idx_system_notifications_user_id ON public.system_notifications(user_id);
CREATE INDEX idx_zone_analytics_zone_date ON public.zone_analytics(zone_name, date);
...
CREATE TRIGGER update_enhanced_support_tickets_updated_at
  BEFORE UPDATE ON public.enhanced_support_tickets
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();
