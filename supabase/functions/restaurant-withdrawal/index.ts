...
serve(async (req) => {
  ...
  try {
    console.log('🍽️ Restaurant withdrawal request received');
    ...
    console.log('👤 Restaurant user authenticated:', user.id);
    ...
    if (paymentMethod !== 'mobile_money') {
      throw new Error("Seuls les retraits mobile money sont supportés");
    }

    if (!paymentDetails.provider || !paymentDetails.phoneNumber) {
      throw new Error("Détails de paiement incomplets");
    }
    ...
    // Récupérer le wallet restaurant (utilise vendor_wallets comme fallback)
    ...
    if (!wallet) {
      console.error('❌ No wallet found for restaurant:', user.id);
      throw new Error('Portefeuille restaurant non trouvé');
    }
    ...
    // ✅ SYSTÈME MANUEL: Créer une demande dans withdrawal_requests
    // Note: user_type doit être 'seller' (contrainte DB) - restaurants sont aussi des vendeurs
    ...
    console.log('✅ Restaurant withdrawal request completed successfully');
    ...
    console.error('❌ Restaurant withdrawal error:', error);
    return new Response(
      JSON.stringify({
        success: false,
