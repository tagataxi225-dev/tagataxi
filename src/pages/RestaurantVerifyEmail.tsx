  ...
  useEffect(() => {
    const checkEmailConfirmation = async () => {
      ...
      if (session) {
        ...
        if (restaurant && !error) {
          setVerified(true);
          toast.success('Email confirmé ! En attente de validation admin...');
          
          setTimeout(() => {
            navigate('/restaurant/auth');
          }, 3000);
        } else {
          console.error('Restaurant profile not found:', error);
          setCompleting(false);
        }
      }
    };
          ...
          <CardContent className="pt-8 text-center space-y-4">
            {verified ? (
              <>
                ...
                <p className="text-muted-foreground">Votre compte est en attente de validation par un administrateur.</p>
                <p className="text-sm text-muted-foreground">Vous recevrez un email dès validation.</p>
              </>
            ) : (
              <>
          ...
          <div className="space-y-4">
            <div className="p-4 bg-accent/50 border border-border rounded-lg">
              ...
              <ol className="text-sm text-muted-foreground mt-2 ml-4 list-decimal space-y-1">
                ...
                <li>Un administrateur validera votre restaurant</li>
...
export default RestaurantVerifyEmail;
