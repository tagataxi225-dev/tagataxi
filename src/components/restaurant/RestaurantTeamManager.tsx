...
import { useRestaurantTeamManagement, RestaurantTeamRole, RESTAURANT_TEAM_ROLE_PERMISSIONS } from '@/hooks/useRestaurantTeamManagement';
import { useAuth } from '@/hooks/useAuth';

const roleConfig: Record<RestaurantTeamRole, { label: string; color: string; icon: React.ElementType }> = {
  admin: { label: 'Administrateur', color: 'bg-destructive/10 text-destructive', icon: Shield },
  chef: { label: 'Chef/Gérant', color: 'bg-orange-500/10 text-orange-600', icon: Users },
  cashier: { label: 'Caissier', color: 'bg-emerald-500/10 text-emerald-600', icon: Eye },
  viewer: { label: 'Observateur', color: 'bg-muted text-muted-foreground', icon: Eye },
};

const permissionLabels = [
  { key: 'menu', label: 'Menu/Plats' },
...
export const RestaurantTeamManager = () => {
  ...
  const {
    ...
    removeMember,
    refresh,
  } = useRestaurantTeamManagement();

  const [showAddDialog, setShowAddDialog] = useState(false);
  const [searchInput, setSearchInput] = useState('');
  const [selectedRole, setSelectedRole] = useState<RestaurantTeamRole>('viewer');
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
            Gérez les collaborateurs de votre restaurant
          </p>
        </div>
          ...
          {isOwner && (
            <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
              <DialogTrigger asChild>
                <Button size="sm" className="gap-1.5 h-8 sm:h-9 text-xs sm:text-sm bg-orange-600 hover:bg-orange-700">
              ...
              <DialogContent>
                ...
                <div className="space-y-4 py-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={selectedRole}
                      onValueChange={(v) => setSelectedRole(v as RestaurantTeamRole)}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {(Object.keys(roleConfig) as RestaurantTeamRole[]).map((key) => {
                          const perms = RESTAURANT_TEAM_ROLE_PERMISSIONS[key];
                          return (
                ...
                <DialogFooter>
                  ...
                  <Button onClick={handleInvite} disabled={actionLoading} className="bg-orange-600 hover:bg-orange-700">
        ...
        <CardContent className="p-0">
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {(Object.keys(RESTAURANT_TEAM_ROLE_PERMISSIONS) as RestaurantTeamRole[]).map((role) => {
                  const perms = RESTAURANT_TEAM_ROLE_PERMISSIONS[role];
          ...
          <div className="px-3 sm:px-4 py-2 border-t border-border/30 flex flex-col sm:flex-row items-start sm:items-center gap-1 sm:gap-4 text-xs text-muted-foreground">
            <span className="flex items-center gap-1"><Check className="h-3 w-3 text-orange-600" /> Accès complet</span>
          ...
          {loading ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-sm text-muted-foreground mt-1 max-w-xs mx-auto">
                Ajoutez des collaborateurs pour déléguer la gestion de votre restaurant
              </p>
              {isOwner && (
                <Button className="mt-4 gap-2 bg-orange-600 hover:bg-orange-700" onClick={() => setShowAddDialog(true)}>
                  <UserPlus className="h-4 w-4" />
                  Ajouter un membre
                </Button>
              )}
            ...
            <div className="space-y-2">
              ...
              {teamAccount && (
                <div className="flex items-center justify-between p-3 rounded-lg bg-orange-500/5 border border-orange-500/10">
                  ...
                  <Badge className="bg-orange-500/10 text-orange-600 text-xs">Propriétaire</Badge>
                </div>
              )}
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      {isOwner ? (
                        <Select
                          value={member.role}
                          onValueChange={(v) => updateMemberRole(member.id, v as RestaurantTeamRole)}
                        >
                          <SelectTrigger className="w-auto h-7 text-xs border-0 bg-transparent p-0 pr-6">
                            <Badge className={cn("text-xs", rc.color)}>{rc.label}</Badge>
                          </SelectTrigger>
                          <SelectContent>
                            {(Object.keys(roleConfig) as RestaurantTeamRole[]).map((k) => (
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                {member.display_name || 'Ce membre'} n'aura plus accès à votre espace restaurant.
                              </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
