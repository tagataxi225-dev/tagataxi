...
export type VendorTeamRole = 'admin' | 'manager' | 'accountant' | 'viewer';
...
export const VENDOR_TEAM_ROLE_PERMISSIONS: Record<VendorTeamRole, {
  label: string;
  products: boolean | 'read';
  orders: boolean | 'read';
  finances: boolean;
  settings: boolean;
}> = {
  admin: { label: 'Administrateur', products: true, orders: true, finances: true, settings: true },
  manager: { label: 'Gestionnaire', products: true, orders: true, finances: false, settings: false },
  accountant: { label: 'Comptable', products: false, orders: 'read', finances: true, settings: false },
  viewer: { label: 'Observateur', products: 'read', orders: 'read', finances: false, settings: false },
};
...
export const useVendorTeamManagement = () => {
  const { user } = useAuth();
    ...
    try {
      ...
      if (!team) {
        ...
        const { data: newTeam, error: createError } = await supabase
          ...
          .insert({
            owner_id: user.id,
            name: vendorProfile?.shop_name ? `Équipe ${vendorProfile.shop_name}` : 'Mon équipe boutique',
          })
          .select()
          .single();
      ...
      if (team) {
        ...
        if (membersData && membersData.length > 0) {
          ...
          // Fetch profiles and clients in parallel for display_name, phone, email
          ...
          setMembers(enriched as VendorTeamMember[]);
        } else {
          setMembers([]);
        }
      }
    } catch (err) {
      console.error('Erreur chargement équipe vendeur:', err);
  ...
  const inviteMember = async (searchInput: string, role: VendorTeamRole): Promise<boolean> => {
    ...
    try {
      ...
      if (isPhone) {
        // Search by phone in clients then profiles
        ...
        // Search by email in clients, chauffeurs, partenaires
        const { data: clientData } = await supabase
          .from('clients')
          .select('user_id')
      ...
      // Insert as active directly (auto-accept)
  ...
  const updateMemberRole = async (memberId: string, newRole: VendorTeamRole) => {
    setActionLoading(true);
    try {
